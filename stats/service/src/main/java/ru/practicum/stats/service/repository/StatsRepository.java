...
public interface StatsRepository extends JpaRepository<StatsHit, Long> {
    @Query("select new ru.practicum.stats.DTO.ViewStats(" +
            "it.app, it.uri, CASE WHEN ?3 = true THEN count(distinct(it.ip)) ELSE count(*) END  as cnt )  from StatsHit as it " +
    ...
    @Query("select new ru.practicum.stats.DTO.ViewStats(" +
            "it.app, it.uri, CASE WHEN ?4 = true THEN count(distinct(it.ip)) ELSE count(*) END as cnt )  from StatsHit as it " +
            "where it.timestamp between ?1 and ?2 " +
            "and it.uri in ?3 " +
            "group by it.app, it.uri " +
            "order by cnt desc")
